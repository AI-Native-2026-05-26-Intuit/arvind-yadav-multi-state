...
- **CI workflow.** [`.github/workflows/web-ci.yml`](.github/workflows/web-ci.yml) is gated by `paths: ['multistate-web/**', '.github/workflows/web-ci.yml']` so backend-only PRs don't pay the frontend tax. The job runs Node 20 with npm cache, then `npm ci → lint → typecheck → test → build`. Keep `package.json` script names in lockstep with the workflow — renaming `test` to `test:ci` here without updating the workflow breaks CI silently.
