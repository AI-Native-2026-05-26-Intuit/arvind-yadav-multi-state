...
class _FakeCrossEncoder:
    ...
    def predict(self, pairs: list[list[str]]) -> list[float]:
        ...
        for pair in pairs:
            ...
            # Reward passages that share distinctive query tokens.
            overlap = sum(1 for tok in query.split() if tok in passage)
            # Strong bonus when the gold California-threshold phrasing appears.
...
def _reset_reranker_singleton(monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep tests hermetic: no HF download; reset module-level cache each test."""
    ...
    # Expose for assertions that need the same instance.
    monkeypatch.setattr(rerank_mod, "_TEST_FAKE_RERANKER", fake, raising=False)


def test_mmr_lambda_one_matches_cosine_topk() -> None:
    # Three candidates with decreasing query similarity; lambda=1.0 == cosine order.
    ...
    # Present out of order so MMR must re-rank.
    candidates = [
        ("c-mid", "mid", 0.0),
        ("c-low", "low", 0.0),
        ("c-best", "best", 0.0),
    ...
    assert [cid for cid, _, _ in picked] == ["c-best", "c-mid", "c-low"]


def test_mmr_lambda_zero_spreads_near_duplicates() -> None:
    q = _unit(np.array([1.0, 0.0], dtype=np.float32))
    # Two near-duplicates aligned with q, plus one orthogonal diverse pick.
    ...
    picked_texts = [txt for _, txt, _ in picked]
    # No two consecutive picks may be near-duplicates under pure diversity.
...
def test_bge_rerank_lifts_gold_from_rank_five() -> None:
    ...
    # Gold sits at retrieval rank 5 (0-based index 4).
    candidates = [*distractors, gold]
    assert candidates[4][0] == "gold"
    ranked, timed_out = bge_rerank(query, candidates, top_k=6, timeout_ms=120_000)
    assert timed_out is False
    top_ids = [cid for cid, _, _ in ranked[:2]]
...
def test_bge_rerank_timeout_fallback_sets_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    candidates = [
        ("c1", "California nexus sales threshold overview for remote sellers.", 0.5),
        ("c2", "New York payroll withholding registration checklist.", 0.4),
    ]
    # Soft timeout: predict always runs; elapsed >> 1 ms → fallback + flag.
...
def test_reranker_loads_once_per_process(monkeypatch: pytest.MonkeyPatch) -> None:
    """Module-level cache: CrossEncoder constructed exactly once."""
    calls = {"n": 0}
    ...
    def _factory() -> _FakeCrossEncoder:
        # Mirror production _get_reranker caching behaviour.
    ...
    candidates = [("c1", "passage one about nexus", 0.5), ("c2", "passage two", 0.4)]
